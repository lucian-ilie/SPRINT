...
class PtoHSP {    //this class aims to compute all HSP and store them in HSP_PAIRS
 public:
  ...
  void change_smer_digit(uint64_t smer1,
                         int digit_pos,
                         HASH_TABLE &ht,
                         int score_needed,
                         set<uint64_t> &similar_smers); //change one digit one a smer. e.g. AND0RB could be changed as AND0RQ for digit 0
  ...
  void extend_HSP(int p1_id,
                  int p2_id,
                  int sta1,
                  int sta2,
                  HASH_TABLE &ht,
                  int score);//extend HSP to both left and right side
  void print_HSP();
...
void PtoHSP::print_HSP() {
  ...
  for (boost::unordered_map<pair<int, int>, set<HSP_pair> >::iterator it = HSP_PAIRS.begin(); it != HSP_PAIRS.end();
       it++) {
...
#pragma omp critical(writeFile)
    ...
    if (ht.hash_table[a].smer != hash_table_default) {
      ...
      for (set<uint64_t>::iterator it = similar_smers.begin(); it != similar_smers.end(); it++) {
        ...
        if ((smer2_index != hash_table_default) && (smer2_index
            >= a)) {//only compare a with the entries that below it, so that we avoid duplicated investigation
...
void PtoHSP::change_smer_digit(uint64_t smer1,
                               int digit_pos,
                               HASH_TABLE &ht,
                               int score_needed,
                               set<uint64_t> &similar_smers) {
  ...
  if (seed_current_digit == 0) {    // the 0 position in seed, just add 0, no need to change anything
    ...
    uint64_t
        smer_cunt_dig = (smer1 & ((uint64_t) 31 << (digit_pos * 5))) >> (digit_pos * 5); //smer's current digit value
    uint64_t y;
    for (int j = 0; j < 20; j++) {
      y = 0;
      if (BLOSUM80[smer_cunt_dig][B80_ordered[smer_cunt_dig][j]] >= score_needed) {
        y = smer1 & ((uint64_t) ~((uint64_t) 31
            << digit_pos * 5));//  1. y's current_digit = 00000(y & ~111011), 2. y's current_digit = j(y | 00001000)
    ...
    for (int j = 0; j < 20; j++) {
      ...
      if (BLOSUM80[smer_cunt_dig][B80_ordered[smer_cunt_dig][j]] >= score_needed) {
        ...
        y = y | ((uint64_t) (B80_ordered[smer_cunt_dig][j]) << (5 * digit_pos));
        similar_smers.insert(y);
        new_score_needed = score_needed + BLOSUM80[smer_cunt_dig_plus1][smer_cunt_dig_plus1]
            - BLOSUM80[smer_cunt_dig][B80_ordered[smer_cunt_dig][j]];
    ...
    for (int a = 0; a < smer1_cnt; a++) {
      for (int b = a + 1; b < smer2_cnt; b++) {
        ...
        if (ONLY_COMPUTE_NEW_PROTEIN) {//check if the proteins are new proteins: if old, skip
          if ((new_proteins.find(p1_id) != new_proteins.end())
              || (new_proteins.find(p2_id) != new_proteins.end())) {//at least one of them needs to be new
            NEED_CHECK = 1;
          } else {
            NEED_CHECK = 0;
          }
        ...
        if (NEED_CHECK) {
          ...
          if ((ht.hash_table[smer1_index].smer_occs[a].pro_id != ht.hash_table[smer2_index].smer_occs[b].pro_id)
              || (ht.hash_table[smer1_index].smer_occs[a].sta_pos != ht.hash_table[smer2_index].smer_occs[b].sta_pos)) {
            // #ifdef PARAL
            // omp_set_lock(&lock);
            // #endif
...
void PtoHSP::put_window(int p1_id,
                        int p2_id,
                        int sta1,
                        int sta2,
                        HASH_TABLE &ht) {    //considering put the k-sized window for every possible position in the future
  int num_check = k_size - (int) ht.seed_len + 1;
  int score = 0;
  for (int pos = 0; pos < num_check; pos++) {
    if ((sta1 - pos >= 0) && (sta2 - pos >= 0) && (sta1 - pos + k_size <= (int) (p_id_seq.at(p1_id).length()))
        && (sta2 - pos + k_size <= (int) (p_id_seq.at(p2_id).length()))) {
...
void PtoHSP::extend_HSP(int p1_id, int p2_id, int sta1, int sta2, HASH_TABLE &ht, int score) {
  ...
  for (int a = 0; a < to_right; a++) {
    current_score = current_score - BLOSUM_score(p_id_seq[p1_id].at(pos_1l), p_id_seq[p2_id].at(pos_2l))
        + BLOSUM_score(p_id_seq[p1_id].at(pos_1r + 1), p_id_seq[p2_id].at(pos_2r + 1));
  ...
  for (int a = 0; a < to_left; a++) {
    current_score = current_score - BLOSUM_score(p_id_seq[p1_id].at(pos_1r), p_id_seq[p2_id].at(pos_2r))
        + BLOSUM_score(p_id_seq[p1_id].at(pos_1l - 1), p_id_seq[p2_id].at(pos_2l - 1));
    if (current_score >= T_kmer) {
      pos_1l--;
      pos_2l--;
      pos_1r--;
    ...
    if (temp1 == ">") {    //read protein info, > p1 and p2
      ...
      if ((p_name_id.find(temp2) != p_name_id.end()) && (p_name_id.find(temp4) != p_name_id.end())) {
        ...
        if (p_name_id.find(temp2) == p_name_id.end())
          cout << "In the HSP file, Protein " << temp2 << " could not be found in the protein sequence file.\n";
        if (p_name_id.find(temp4) == p_name_id.end())
          cout << "In the HSP file, Protein " << temp4 << " could not be found in the protein sequence file.\n";
      }
